...
export function validarLimiteAusencias(
  ...
  novaDataFim: string,
  todosFuncionarios: RhEmployee[],
  limiteMaximo: number = 8
): { days: DailyAbsence[]; isValid: boolean; exceedDays: string[] } {
  // Gera todas as datas do período
  const start = new Date(novaDataInicio);
  const end = new Date(novaDataFim);
  const daily: Record<string, { count: number; employees: string[] }> = {};

  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    const dateStr = d.toISOString().split('T')[0];
    daily[dateStr] = { count: 0, employees: [] };
  }

  // Verifica ausências existentes
  todosFuncionarios.forEach(emp => {
    if (emp.statusFuncionario !== 'Ativo') return;
    
    // Considerar férias
    emp.periodosFerias?.forEach(p => {
      const pStart = new Date(p.dataInicio);
      const pEnd = new Date(p.dataFim);
      for (let d = new Date(pStart); d <= pEnd; d.setDate(d.getDate() + 1)) {
        const dStr = d.toISOString().split('T')[0];
        if (daily[dStr]) {
          daily[dStr].count++;
          daily[dStr].employees.push(emp.nome);
        }
      }
    });

    // TODO: Adicionar viagens, treinamentos, etc, se existirem na modelagem
  });

  const days: DailyAbsence[] = Object.keys(daily).sort().map(k => ({
    date: k,
    count: daily[k].count,
    employees: daily[k].employees
  }));

  const exceedDays = days.filter(d => d.count >= limiteMaximo).map(d => d.date);
  
  return {
    days,
    isValid: exceedDays.length === 0,
    exceedDays
  };
}
