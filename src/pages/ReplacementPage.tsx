    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-sky-100 pb-4">
        ...
        <p className="text-xs text-slate-400 font-medium mt-1">
          Analise a cobertura Funcionário por idioma e planeje contratações ou renovações com antecedência de até 180 dias.
        </p>
      </div>

      {/* Estatísticas de Cobertura */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 bg-white border border-sky-100 shadow-[0_4px_20px_-4px_rgba(23,105,170,0.04)] rounded-2xl flex items-center justify-between">
          <div>
          ...
          <div className="p-3 bg-sky-50 text-yinmn rounded-xl">
          ...
          <table className="w-full min-w-[900px] border-collapse text-xs">
            <thead>
              <tr className="border-b border-sky-100 text-slate-400 font-black uppercase text-[10px] bg-[#FAF9FF]">
                <th className="py-3 px-3 text-left bg-sky-50/25 rounded-l-xl">Idioma</th>
                ...
                <th className="py-3 px-3 text-center bg-sky-50 text-cadet font-extrabold">Saem 180 d</th>
            ...
            <tbody className="divide-y divide-sky-100">
              {languageData.map((data) => {
                ...
                if (data.situacao === "Crítico") {
                  ...
                  statusBadgeStyle = "bg-sky-50 text-yinmn border-sky-100 font-bold";
                }

                return (
                  <tr key={data.idioma} className="hover:bg-sky-50/10 transition-colors duration-200">
                    {/* Idioma */}
                    ...
                    <td className="py-4 px-3 text-center font-bold text-yinmn bg-sky-50/10">{data.saida180}</td>

                    {/* Cobertura Restante */}
                    <td className="py-4 px-3">
                      <div className="flex flex-col items-center gap-1.5">
                        <span className="font-extrabold text-slate-700">{data.coberturaRestante}%</span>
                        <div className="w-16 h-1.5 bg-[#FAF9FF] border border-sky-100/35 rounded-full overflow-hidden">
                          <div
      ...
      <div className="bg-oxford text-white p-6 rounded-2xl border border-cadet/30 shadow-lg space-y-4">
        <h4 className="text-sm font-black uppercase tracking-wider text-jordy flex items-center gap-2">
          <UserPlus2 className="w-4 h-4 text-jordy" />
          <span>Diretrizes e Recomendações Gerais de Contratação</span>
        </h4>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs leading-relaxed">
          <div className="p-4 bg-cadet/30 rounded-xl border border-jordy/20 space-y-2">
            <h5 className="font-bold text-sky-100 flex items-center gap-1.5 text-xs">
              <UserCheck className="w-4 h-4 text-jordy" />
              <span>Mitigação de Riscos Críticos</span>
            </h5>
            <p className="text-sky-100/80 text-[11px] font-medium leading-relaxed">
          ...
          <div className="p-4 bg-cadet/30 rounded-xl border border-jordy/20 space-y-2">
            <h5 className="font-bold text-sky-100 flex items-center gap-1.5 text-xs">
              <CheckCircle2 className="w-4 h-4 text-jordy" />
